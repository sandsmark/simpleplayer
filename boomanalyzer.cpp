...
{
    /*QWidget *o, *box = new QWidget(this, Qt::Window);
    ...
    l = new QLabel("Bar fall-rate:", box);
    o = new QSlider(100, 2000, 100, v, Qt::Horizontal, box);
    m = new QSpinBox(100, 2000, 1, box);
    m->setValue(v);

    layout->addWidget(l, 0, 0);
    layout->addWidget(o, 0, 1);
    ...
    l = new QLabel("Peak acceleration: ", box);
    o = new QSlider(1000, 1300, 50, v, Qt::Horizontal, box);
    m = new QSpinBox(1000, 1300, 1, box);
    m->setValue(v);

    layout->addWidget(l, 1, 0);
    layout->addWidget(o, 1, 1);
    layout->addWidget(m, 1, 2);
...
void BoomAnalyzer::analyze(const QVector<float> &scope)
{
    eraseCanvas();
    ...
    for( uint i = 0, x = 0, y; i < BAND_COUNT; ++i, x += COLUMN_WIDTH+1 )
    {
        ...
        //p.setPen(Amarok::ColorScheme::Foreground); //TODO
        p.setPen(Qt::blue);
        p.drawRect( x, y, COLUMN_WIDTH, height() - y );

        y = height() - uint(peak_height[i]);
        //p.setPen( Amarok::ColorScheme::Text ); TODO
        p.setPen(Qt::black);
